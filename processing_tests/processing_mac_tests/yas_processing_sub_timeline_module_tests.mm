...
- (void)test_process_signal {
...

  {
    ...
    main_track_2.insert_module(time::range{0, 4},
                               std::move(sub_timeline_module));
  }
...
- (void)test_process_overwrite_signal {
...

  {
    ...
    main_track_2.insert_module(time::range{1, 2},
                               std::move(sub_timeline_module));
  }
...
- (void)test_process_number {
...

  {
    ...
    while (yas_each_next(each)) {
      ...
      left_module.connect_output(to_connector_index(constant::output::value),
                                 0);

      auto right_module = make_number_module(int8_t(8));
      right_module.connect_output(to_connector_index(constant::output::value),
                                  1);
...

  {
    ...
    auto &main_track_2 = main_timeline.add_track(2);
    main_track_2.insert_module(time::range{1, 1},
                               std::move(sub_timeline_module));
  }
...

  {
    ...
    auto const left_events =
        left_channel.filtered_events<int8_t, number_event>();
    auto const right_events =
        right_channel.filtered_events<int8_t, number_event>();
...

  {
    ...
    XCTAssertEqual(
        cast<number_event>(channel.events().cbegin()->second).get<int8_t>(),
        15);
  }
}
...
- (void)test_process_overwrite_number {
...

  {
    ...
    while (yas_each_next(each)) {
      ...
      main_module.connect_output(to_connector_index(constant::output::value),
                                 0);

      auto &main_track = main_timeline.add_track(0);
      main_track.insert_module(time::range{yas_each_index(each), 1},
                               std::move(main_module));
    }
  }

  {
    ...
    while (yas_each_next(each)) {
      ...
      auto &sub_track = sub_timeline.add_track(0);
      sub_track.insert_module(time::range{1 + yas_each_index(each), 2},
                              std::move(sub_module));
    }
  }
