...
@interface yas_processing_time_range_tests : XCTestCase

@end

@implementation yas_processing_time_range_tests
...
- (void)test_intersect_overlapped {
  XCTAssertEqual((time::range{0, 1}.intersect(time::range{0, 1})),
                 (time::range{0, 1}));
  XCTAssertEqual((time::range{0, 2}.intersect(time::range{1, 2})),
                 (time::range{1, 1}));
  XCTAssertEqual((time::range{0, 2}.intersect(time::range{1, 1})),
                 (time::range{1, 1}));
  XCTAssertEqual((time::range{0, 1}.intersect(time::range{0, 2})),
                 (time::range{0, 1}));
}
