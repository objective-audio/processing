...
- (void)test_process {
...

  {
    ...
    XCTAssertEqual(signal_events.cbegin()->first,
                   (time::range{0, process_length}));
...

  {
    ...
    XCTAssertEqual(signal_events.size(), 1);
    XCTAssertEqual(signal_events.cbegin()->first,
                   (time::range{process_length, process_length}));
...
- (void)test_connect_output {
  ...
  XCTAssertEqual(connectors.size(), 1);
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(number_to_signal::output::signal));
