...
- (void)test_sample_type {
  XCTAssertTrue(proc::make_signal_event<int8_t>(1).sample_type() ==
                typeid(int8_t));
  XCTAssertTrue(proc::make_signal_event<double>(0.0).sample_type() ==
                typeid(double));
  XCTAssertTrue(proc::make_signal_event<boolean>(1).sample_type() ==
                typeid(boolean));
}
...
- (void)test_copy_in_range_top {
  ...
  auto const copied_signal_event =
      src_signal_event.copy_in_range(time::range{0, 2});
  auto const &copied_vec = copied_signal_event.vector<int8_t>();
...
- (void)test_copy_in_range_middle {
  ...
  auto const copied_signal_event =
      src_signal_event.copy_in_range(time::range{1, 1});
  auto const &copied_vec = copied_signal_event.vector<int8_t>();
...
- (void)test_copy_in_range_tail {
  ...
  auto const copied_signal_event =
      src_signal_event.copy_in_range(time::range{1, 2});
  auto const &copied_vec = copied_signal_event.vector<int8_t>();
...
- (void)test_combined {
  ...
  auto const combined = main_signal_event.combined(
      time::range{1, 1},
      {std::make_pair(time::range{0, 1}, std::move(sub_signal_event_0)),
