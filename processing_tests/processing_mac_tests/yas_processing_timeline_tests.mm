...
- (void)test_process {
  ...
  auto send_handler1 = [](proc::time::range const &time_range,
  ...
  auto processor1 =
      make_send_signal_processor<int16_t>({std::move(send_handler1)});
  ...
  auto receive_processor2 =
      make_receive_signal_processor<int16_t>({std::move(receive_handler2)});
  auto send_processor2 =
      make_send_signal_processor<int16_t>({std::move(send_handler2)});
  auto module2 =
      module{{std::move(receive_processor2), std::move(send_processor2)}};
...

  {
    ...
    auto const &vec =
        cast<proc::signal_event>(events.cbegin()->second).vector<int16_t>();
...
- (void)test_process_offline {
  ...
  timeline.process(time::range{0, process_length}, sync_source{1, 2},
                   [&ch_idx, &called](time::range const &time_range,
                                      stream const &stream, bool &) {
                     ...
                     called.emplace_back(
                         std::make_pair(pair.first.get<time::range>(),
                                        signal.vector<int8_t>()));
                   });
...
- (void)test_stop_process_offline {
  ...
  timeline.process(
      time::range{0, process_length}, sync_source{1, 1},
