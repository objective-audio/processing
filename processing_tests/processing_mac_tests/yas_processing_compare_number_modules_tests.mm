...
- (void)test_is_equal {
  ...
  auto stream =
      test::make_number_stream(process_length, left_data, time::range{1, 3}, 0,
                               right_data, time::range{2, 3}, 1);
  ...
  auto const events =
      stream.channel(2).filtered_events<boolean, number_event>();
...
- (void)test_is_greater {
  ...
  auto stream =
      test::make_number_stream(process_length, left_data, time::range{1, 7}, 0,
                               right_data, time::range{4, 6}, 1);
...
- (void)test_connect_input {
  ...
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(compare::input::left));
...
- (void)test_connect_output {
  ...
  XCTAssertEqual(connectors.size(), 1);
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(compare::output::result));
