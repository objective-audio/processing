...
- (void)test_send_signal_processor {
  ...
  auto clear = [&called_time, &called_co_idx, &called_ch_idx,
                &called_sample_rate, &called_slice_length]() {
  ...
  auto handler = [&called_time, &called_co_idx, &called_ch_idx,
                  &called_sample_rate, &called_slice_length](
                     proc::time::range const &time_range,
                     sync_source const &sync_src, channel_index_t const ch_idx,
  ...
  auto processor =
      proc::make_send_signal_processor<int64_t>(std::move(handler));
...

  {
    ...
    auto const &vec = cast<proc::signal_event>(
                          stream.channel(ch_idx).events().cbegin()->second)
                          .vector<int64_t>();
...
- (void)test_receive_signal_processor {
  ...
  auto clear = [&called_time, &called_co_idx, &called_ch_idx,
                &called_sample_rate, &called_slice_length, &called_signal]() {
    called_time = make_any_time();
    called_co_idx = 0;
    called_ch_idx = 0;
    called_sample_rate = 0;
...
- (void)test_receive_and_send_signal_processor {
  ...
  auto receive_handler = [&process_signal](proc::time::range const &time_range,
                                           sync_source const &,
  ...
  auto send_handler =
      [&process_signal](proc::time::range const &time_range,
                        sync_source const &, channel_index_t const ch_idx,
  ...
  auto receive_processor =
      make_receive_signal_processor<int16_t>(std::move(receive_handler));
  auto send_processor =
      proc::make_send_signal_processor<int16_t>(std::move(send_handler));
  auto module =
      proc::module{{std::move(receive_processor), std::move(send_processor)}};
  ...
  auto const &send_vec =
      cast<proc::signal_event>(send_channel.events().cbegin()->second)
          .vector<int16_t>();
  ...
  auto const &receive_vec =
      cast<proc::signal_event>(receive_channel.events().cbegin()->second)
          .vector<int16_t>();
