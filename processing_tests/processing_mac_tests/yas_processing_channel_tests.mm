...
- (void)test_create_channel_from_events {
  ...
  events.emplace(
      std::make_pair(make_frame_time(10), make_number_event(int8_t(100))));
...
- (void)test_insert_event {
  ...
  channel.insert_event(std::move(uint_range_time),
                       signal_event{std::move(uint_vec)});
  channel.insert_event(std::move(float_range_time),
                       signal_event{std::move(float_signal)});
...
- (void)test_invalid_insert_event {
  ...
  XCTAssertThrows(
      channel.insert_event(proc::time{0, 1}, std::move(signal_event)));
}
...
- (void)test_typed_erase_event {
  ...
  auto const float_signal_events =
      channel.filtered_events<float, signal_event>();
  ...
  auto const double_signal_events =
      channel.filtered_events<double, signal_event>();
  XCTAssertEqual(double_signal_events.size(), 1);

  auto const int8_number_events =
      channel.filtered_events<int8_t, number_event>();
...
- (void)test_combine_signal_event {
  ...
  auto combined_pair =
      channel.combine_signal_event(time::range{2, 2}, std::move(insert_signal));
