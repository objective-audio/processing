...
- (void)test_insert_data {
    ...
    auto uint_data = processing::make_data<uint32_t>(4);
    auto &uint_raw = uint_data.raw<uint32_t>();
    uint_raw[0] = 10;
    uint_raw[1] = 11;
    uint_raw[2] = 12;
    uint_raw[3] = 13;

    auto float_data = processing::make_data<float>(2);
    auto &float_raw = float_data.raw<float>();
    float_raw[0] = 2.0f;
    float_raw[1] = 4.0f;

    processing::time_range uint_time_range{.start_frame = 16, .length = 1024};
    processing::time_range float_time_range{.start_frame = 8, .length = 2048};
    ...
    for (auto &pair : channel.datas()) {
        ...
        if (idx == 0) {
            XCTAssertTrue((time_range == processing::time_range{.start_frame = 8, .length = 2048}));
            XCTAssertTrue(data.sample_type() == typeid(float));
            auto const &raw = data.raw<float>();
            XCTAssertEqual(raw.size(), 2);
            XCTAssertEqual(raw[0], 2.0f);
            XCTAssertEqual(raw[1], 4.0f);
        } else if (idx == 1) {
            XCTAssertTrue((time_range == processing::time_range{.start_frame = 16, .length = 1024}));
            XCTAssertTrue(data.sample_type() == typeid(uint32_t));
            auto const &raw = data.raw<uint32_t>();
...
- (void)test_insert_same_time_range_data {
    ...
    auto uint_data = processing::make_data<uint32_t>(1);
    processing::time_range uint_time_range{.start_frame = 1, .length = 2};
    
    auto float_data = processing::make_data<float>(1);
...
- (void)test_const_data {
    ...
    auto data = processing::make_data<float>(1);
    data.raw<float>()[0] = 1.0f;

    channel.insert_data(processing::time_range{.start_frame = 10, .length = 20}, std::move(data));
    ...
    for (auto const &pair : const_channel.datas()) {
        ...
        XCTAssertTrue((const_time_range == processing::time_range{.start_frame = 10, .length = 20}));
        XCTAssertEqual(const_data.raw<float>()[0], 1.0f);
    }
}
