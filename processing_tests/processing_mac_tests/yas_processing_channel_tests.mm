...
- (void)test_insert_buffer {
    ...
    processing::time_range uint_time_range{.frame = 16, .length = 4};
    processing::time_range float_time_range{.frame = 8, .length = 2};
    ...
    for (auto &pair : channel.buffers()) {
        ...
        if (idx == 0) {
            XCTAssertTrue((time_range == processing::time_range{.frame = 8, .length = 2}));
            ...
            XCTAssertTrue((time_range == processing::time_range{.frame = 16, .length = 4}));
...
- (void)test_insert_same_time_range_buffer {
    ...
    processing::time_range uint_time_range{.frame = 1, .length = 2};

    auto float_buffer = make_buffer<float>(2);
    processing::time_range float_time_range{.frame = 1, .length = 2};
...
- (void)test_const_buffer {
    ...
    channel.insert_buffer(processing::time_range{.frame = 10, .length = 1}, std::move(buffer));
    ...
    for (auto const &pair : const_channel.buffers()) {
        ...
        XCTAssertTrue((const_time_range == processing::time_range{.frame = 10, .length = 1}));
...
- (void)test_invalid_insert_buffer {
    ...
    XCTAssertThrows(channel.insert_buffer(processing::time_range{.length = 1}, std::move(buffer)));
}
