...
- (void)test_process_signal_diff_channel {
  ...
  constant_module1.connect_output(to_connector_index(constant::output::value),
                                  0);

  auto constant_module2 = make_signal_module(int8_t(2));
  constant_module2.connect_output(to_connector_index(constant::output::value),
                                  0);
...
- (void)test_process_signal_same_channel {
  ...
  auto const filtered_events =
      channel.filtered_events<double, proc::signal_event>();
...
- (void)test_connect_input {
  ...
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(cast::input::value));
...
- (void)test_connect_output {
  ...
  XCTAssertEqual(connectors.size(), 1);
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(cast::output::value));
