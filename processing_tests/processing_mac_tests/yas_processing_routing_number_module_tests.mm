...
- (void)test_move_number_process {
  ...
  auto stream = test::make_number_stream(process_length, data,
  ...
  auto const &output_events =
      stream.channel(output_ch_idx).filtered_events<int8_t, number_event>();
...
- (void)test_move_number_process_part {
  ...
  auto const &unprocessed_events =
      stream.channel(input_ch_idx).filtered_events<int8_t, number_event>();
...
- (void)test_connect_input {
  ...
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(routing::input::value));
...
- (void)test_connect_output {
  ...
  XCTAssertEqual(connectors.size(), 1);
  XCTAssertEqual(connectors.cbegin()->first,
                 to_connector_index(routing::output::value));
