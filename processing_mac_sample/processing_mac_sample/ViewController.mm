...
- (IBAction)makeAudioFile:(id)sender {
  ...
  if ([panel runModal] == NSFileHandlingPanelOKButton) {
    ...
    auto wave_settings =
        audio::wave_file_settings(double(sample_rate), 1, bits);
    auto create_result =
        audio::make_created_file({.file_url = (__bridge CFURLRef)panel.URL,
                                  .file_type = audio::file_type::wave,
                                  .settings = wave_settings});

    if (!create_result) {
      std::cout << __PRETTY_FUNCTION__
                << " - error:" << to_string(create_result.error()) << std::endl;
      return;
    }
    ...
    if (auto &second_track = timeline.add_track(trk_idx++)) {
      auto second_module =
          make_signal_module<float>(generator::kind::second, 0);
      second_module.connect_output(to_connector_index(generator::output::value),
                                   0);
    ...
    if (auto &multiply_track = timeline.add_track(trk_idx++)) {
      ...
      multiply_module.connect_output(to_connector_index(math2::output::result),
                                     0);
    ...
    if (auto &env_track = timeline.add_track(trk_idx++)) {
      envelope::anchors_t<float> anchors{{0, startGainValue},
                                         {process_range.length, endGainValue}};
    ...
    if (auto &level_track = timeline.add_track(trk_idx++)) {
      ...
      level_module.connect_output(to_connector_index(constant::output::value),
                                  1);
    ...
    timeline.process(
        ...
        [file, buffer, &write_failed](time::range const &current_range,
                                      stream const &stream,
                                      bool &stop) mutable {
...
- (void)updateSlider:(NSSlider *)slider
  ...
  double const doubleValue =
      formatter ? formatter(textField.stringValue) : textField.doubleValue;
