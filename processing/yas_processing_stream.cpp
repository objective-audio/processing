...
struct processing::stream::impl : base::impl {
    ...
    std::unordered_map<int64_t, processing::channel> _channels;
...
void processing::stream::insert_channel(int64_t const channel) {
...
processing::channel const &processing::stream::channel(uint64_t const channel) const {
    return impl_ptr<impl>()->_channels.at(channel);
}

processing::channel &processing::stream::channel(uint64_t const channel) {
    return impl_ptr<impl>()->_channels.at(channel);
}
