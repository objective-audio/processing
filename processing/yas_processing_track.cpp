...
proc::track::impl::impl() {
}

proc::track::impl::impl(modules_map_t &&modules) : _modules_holder(to_modules_holders(std::move(modules))) {
}

void proc::track::impl::push_back_module(module &&module, time::range &&range) {
...
void proc::track::impl::insert_module(module &&module, std::size_t const idx, time::range &&range) {
...
bool proc::track::impl::erase_module(module const &erasing) {
...
bool proc::track::impl::erase_module(module const &erasing, time::range const &range) {
    if (this->_modules_holder.has_value(range)) {
...
bool proc::track::impl::erase_module_at(std::size_t const idx, time::range const &range) {
    if (this->_modules_holder.has_value(range)) {
        auto &modules = this->_modules_holder.at(range);
        if (idx < modules.size()) {
...
void proc::track::impl::erase_modules_for_range(time::range const &range) {
    this->_modules_holder.erase_for_key(range);
}

void proc::track::impl::process(time::range const &time_range, stream &stream) {
    for (auto &pair : this->_modules_holder.raw()) {
...
std::optional<proc::time::range> proc::track::impl::total_range() {
...
proc::track proc::track::impl::copy() {
    return proc::track{copy_to_modules(this->_modules_holder.raw())};
}

void proc::track::impl::broadcast(event_t const &value) {
    this->_modules_holder.sendable()->broadcast(value);
}

void proc::track::impl::send_value_to_target(event_t const &value, std::uintptr_t const key) {
    this->_modules_holder.sendable()->send_value_to_target(value, key);
}

void proc::track::impl::erase_joint(std::uintptr_t const key) {
    this->_modules_holder.sendable()->erase_joint(key);
}

void proc::track::impl::fetch_for(chaining::any_joint const &joint) {
    this->_modules_holder.sendable()->fetch_for(joint);
}

chaining::chain_unsync_t<proc::track::event_t> proc::track::impl::chain_unsync() {
    return this->_modules_holder.sendable()->chain_unsync();
}

chaining::chain_sync_t<proc::track::event_t> proc::track::impl::chain_sync() {
...
std::shared_ptr<weakable_impl> proc::track::weakable_impl_ptr() const {
    return this->impl_ptr<impl>();
}
