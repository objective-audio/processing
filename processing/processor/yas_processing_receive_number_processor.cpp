...
template <typename T>
proc::processor_f
proc::make_receive_number_processor(proc::receive_number_process_f<T> handler) {
  return
          ...
          for (auto const &connector_pair : input_connectors) {
            ...
            if (stream.has_channel(ch_idx)) {
              ...
              auto const filtered_events =
                  channel.filtered_events<T, proc::number_event>();
...
proc::make_receive_number_processor(proc::receive_number_process_f<double>);
template proc::processor_f
proc::make_receive_number_processor(proc::receive_number_process_f<float>);
...
template proc::processor_f
    proc::make_receive_number_processor(proc::receive_number_process_f<int8_t>);
template proc::processor_f proc::make_receive_number_processor(
