...
public:
  ...
  template <typename T> using value_map_t = std::multimap<time::frame::type, T>;

  template <typename T> explicit number_event(T);
...
template <typename T> number_event make_number_event(T);
} // namespace yas::proc
