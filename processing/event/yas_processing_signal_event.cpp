...
proc::signal_event
proc::signal_event::copy_in_range(time::range const &range) const {
  return this->impl_ptr<impl>()->copy_in_range(range);
}

std::vector<std::pair<proc::time::range, proc::signal_event>>
proc::signal_event::cropped(time::range const &range) const {
  return this->impl_ptr<impl>()->cropped(range);
}

proc::signal_event::pair_t
proc::signal_event::combined(time::range const &insert_range,
                             pair_vector_t event_pairs) {
