...
void proc::timeline::process(time::range const &time_range,
                             sync_source const &sync_src,
  ...
  while (frame < time_range.next_frame()) {
    ...
    time::range const current_time_range = time::range{
        frame, static_cast<length_t>(sync_next_frame < end_next_frame
                                         ? sync_next_frame - frame
