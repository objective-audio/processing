...
namespace yas::proc {
static void connect(connector_map_t &connectors, connector_index_t const idx,
...
static void disconnect(connector_map_t &connectors,
                       connector_index_t const idx) {
  if (connectors.count(idx) > 0) {
    connectors.erase(idx);
  }
}
...
struct proc::module::impl : base::impl {
  ...
  connector_map_t &input_connectors() { return this->_input_connectors; }

  connector_map_t &output_connectors() { return this->_output_connectors; }

  processors_t &processors() { return this->_processors; }

  void process(proc::time::range const &time_range, stream &stream) {
    for (auto &processor : this->_processors) {
      if (processor) {
        processor(time_range, this->_input_connectors, this->_output_connectors,
                  stream);
      }
    }
  }
...
proc::module::module(processors_t processors)
    : base(std::make_shared<impl>(std::move(processors))) {}
...
void proc::module::connect_input(connector_index_t const co_idx,
                                 channel_index_t const ch_idx) {
  connect(this->impl_ptr<impl>()->input_connectors(), co_idx, ch_idx);
}

void proc::module::connect_output(connector_index_t const co_idx,
                                  channel_index_t const ch_idx) {
