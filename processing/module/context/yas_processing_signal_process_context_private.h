...
signal_process_context<T, N>::signal_process_context() {
  ...
  while (yas_each_next(each)) {
    this->_inputs.emplace_back(
        std::make_pair(proc::time{nullptr}, signal_event{std::vector<T>(0)}));
  }
}
...
template <typename T, std::size_t N>
typename signal_process_context<T, N>::pair_vector_t const &
signal_process_context<T, N>::inputs() const {
  return this->_inputs;
}

template <typename T, std::size_t N>
void signal_process_context<T, N>::copy_data_from(T const *ptr,
...
template <typename T, std::size_t N>
void signal_process_context<T, N>::set_time(proc::time time,
                                            std::size_t const idx) {
