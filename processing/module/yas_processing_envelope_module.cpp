...
template <typename T> struct context {
  context(anchors_t<T> &&anchors)
      : _anchors(std::move(anchors)), _prev(_anchors.cbegin()),
        _next(_anchors.cbegin()), _end(_anchors.cend()) {}

  void reset(time::range const &current_range) {
    if (this->_last_range &&
        this->_last_range->next_frame() != current_range.frame) {
  ...
  T value(frame_index_t const env_frame) {
    ...
    if (this->_next == this->_end) {
      ...
      double const rate =
          double(env_frame - prev_frame) / (this->_next->first - prev_frame);
      return static_cast<T>((this->_next->second - prev_value) * rate +
                            prev_value);
    }
  }
...
template <typename T>
proc::module proc::envelope::make_signal_module(anchors_t<T> anchors,
...
template proc::module proc::envelope::make_signal_module(anchors_t<double>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<float>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<int64_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<int32_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<int16_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<int8_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<uint64_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<uint32_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<uint16_t>,
                                                         frame_index_t const);
template proc::module proc::envelope::make_signal_module(anchors_t<uint8_t>,
                                                         frame_index_t const);
