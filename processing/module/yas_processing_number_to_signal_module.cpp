...
template <typename T> proc::module proc::make_number_to_signal_module() {
  ...
  auto remove_processor = make_remove_number_processor<T>(
      {to_connector_index(number_to_signal::input::number)});

  auto send_processor = make_send_signal_processor<T>(
      [context, out_each = fast_each<T *>{}](
  ...
  return module{{std::move(prepare_processor), std::move(receive_processor),
...
void yas::connect(proc::module &module,
                  proc::number_to_signal::input const &input,
                  proc::channel_index_t const &ch_idx) {
  module.connect_input(proc::to_connector_index(input), ch_idx);
}

void yas::connect(proc::module &module,
                  proc::number_to_signal::output const &output,
                  proc::channel_index_t const &ch_idx) {
