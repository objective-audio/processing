...
template <typename In, typename Out> proc::module make_signal_module() {
  ...
  auto receive_processor = proc::make_receive_signal_processor<In>(
      [context](proc::time::range const &time_range, sync_source const &,
  ...
  auto remove_processor = proc::make_remove_signal_processor<In>(
      {to_connector_index(input::value)});

  auto send_processor = proc::make_send_signal_processor<Out>(
      [context, out_each = fast_each<Out *>{}](
        ...
        if (co_idx == output_co_idx) {
          ...
          auto const src_offset =
              src_time ? time_range.frame - src_time.get<time::range>().frame
                       : 0;
          auto const &src_length =
              src_time ? src_time.get<time::range>().length : 0;
...
template <typename In, typename Out> proc::module make_number_module() {
  ...
  auto receive_processor = proc::make_receive_number_processor<In>(
      [context](proc::time::frame::type const &frame,
  ...
  auto remove_processor = proc::make_remove_number_processor<In>(
      {to_connector_index(input::value)});

  auto send_processor = [context](time::range const &current_time_range,
                                  connector_map_t const &,
    ...
    if (output_connectors.count(out_co_idx) > 0) {
      ...
      auto &channel = stream.has_channel(ch_idx) ? stream.channel(ch_idx)
                                                 : stream.add_channel(ch_idx);
      for (auto const &input_pair : context->inputs()) {
        channel.insert_event(make_frame_time(input_pair.first),
                             number_event{static_cast<Out>(
                                 *input_pair.second.values[input_co_idx])});
      }
    }
  };
