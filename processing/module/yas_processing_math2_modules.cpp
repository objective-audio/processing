...
proc::module proc::make_signal_module(math2::kind const kind) {
  ...
  auto receive_processor = proc::make_receive_signal_processor<T>(
      [context](time::range const &time_range, sync_source const &,
        ...
        if (co_idx == to_connector_index(input::left) ||
            co_idx == to_connector_index(input::right)) {
      ...
      [context, kind, out_each = fast_each<T *>{}](
        ...
        if (co_idx == to_connector_index(output::result)) {
          ...
          auto const left_offset =
              left_time ? time_range.frame - left_time.get<time::range>().frame
                        : 0;
          auto const right_offset =
              right_time
                  ? time_range.frame - right_time.get<time::range>().frame
                  : 0;
          auto const &left_length = left_time
                                        ? left_time.get<time::range>().length
                                        : constant::zero_length;
          auto const &right_length = right_time
                                         ? right_time.get<time::range>().length
                                         : constant::zero_length;
          ...
          while (yas_each_next(out_each)) {
            ...
            auto const &left_value = (left_idx >= 0 && left_idx < left_length)
                                         ? left_ptr[left_idx]
                                         : 0;
            auto const &right_value =
                (right_idx >= 0 && right_idx < right_length)
                    ? right_ptr[right_idx]
                    : 0;
            ...
            case kind::multiply:
              ...
              break;
            case kind::divide:
              yas_each_value(out_each) = (left_value == 0 || right_value == 0)
                                             ? 0
  ...
  return proc::module{{std::move(prepare_processor),
                       std::move(receive_processor),
        ...
        if (co_idx == to_connector_index(output::result)) {
          ...
          for (auto const &input_pair : context->inputs()) {
            ...
            case kind::multiply:
              ...
              break;
            case kind::divide:
              result_value = (left_value == 0 || right_value == 0)
                                 ? 0
                                 : left_value / right_value;
              break;
...
void yas::connect(proc::module &module, proc::math2::input const &input,
                  proc::channel_index_t const &ch_idx) {
  module.connect_input(proc::to_connector_index(input), ch_idx);
}

void yas::connect(proc::module &module, proc::math2::output const &output,
                  proc::channel_index_t const &ch_idx) {
