...
proc::time::range::range(frame_index_t const frame, length_t const length)
    : frame(frame), length(length) {
  if (this->length == 0) {
...
bool proc::time::range::is_contain(any::type const &any) const { return true; }
...
bool proc::time::range::is_overlap(time::range const &rhs) const {
  ...
  return std::max(this->frame, rhs.frame) <
         std::min(next_frame(), rhs.next_frame());
}
...
std::experimental::optional<proc::time::range>
proc::time::range::intersect(time::range const &rhs) const {
...
std::experimental::optional<proc::time::range>
proc::time::range::combine(time::range const &other) const {
...
std::vector<proc::time::range>
proc::time::range::crop(range const &other) const {
  ...
  if (auto const cropped_ragne_opt = intersect(other)) {
    ...
    if (this->frame < cropped_range.frame) {
      vec.emplace_back(time::range{
          this->frame,
          static_cast<length_t>(cropped_range.frame - this->frame)});
    }
    ...
    if (cropped_next_frame < current_next_frame) {
      vec.emplace_back(time::range{
          cropped_next_frame,
          static_cast<length_t>(current_next_frame - cropped_next_frame)});
    }
  }
...
bool proc::time::any::operator==(time::any const &) const { return true; }

bool proc::time::any::operator!=(time::any const &) const { return false; }
...
template <typename T> struct proc::time::impl : impl_base {
...
proc::time::time(range range)
    : base(std::make_shared<impl<time::range>>(std::move(range))) {}

proc::time::time(frame_index_t const frame)
    : base(std::make_shared<impl<time::frame>>(frame)) {}
...
bool proc::time::is_range_type() const { return this->type() == typeid(range); }

bool proc::time::is_frame_type() const { return this->type() == typeid(frame); }

bool proc::time::is_any_type() const { return this->type() == typeid(any); }
...
template <typename T> typename T::type const &proc::time::get() const {
...
template proc::time::range::type const &
proc::time::get<proc::time::range>() const;
template proc::time::frame::type const &
proc::time::get<proc::time::frame>() const;
...
std::string yas::to_string(proc::time::range const &range) {
  return "{" + std::to_string(range.frame) + ", " +
         std::to_string(range.length) + "}";
}

#pragma mark - private

std::shared_ptr<proc::time::impl<proc::time::any>> const &
proc::time::any_impl_ptr() {
...
proc::time proc::make_range_time(frame_index_t const frame,
...
proc::time proc::make_any_time() { return proc::time{}; }
